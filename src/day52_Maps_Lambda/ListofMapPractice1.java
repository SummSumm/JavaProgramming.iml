...
public class ListofMapPractice1 {
    public static void main(String[] args) {
        ...
        System.out.println(scrumTeams.size()); // sizeı 2 çünkü 2 tane map var içinde


        //print names of all employees
        for (Map<String, String> eachTeam : scrumTeams) {
            for (Map.Entry<String, String> eachPerson : eachTeam.entrySet()) {
                System.out.println("each employee = " + eachPerson.getKey() + eachPerson.getValue());
            }
        }
        System.out.println("-----------------------------------------------------------");
        // get the Scrum master names (Keys of SM values)

        for (Map<String, String> eachTeam : scrumTeams) {
            for (Map.Entry<String, String> eachPerson : eachTeam.entrySet()) {
                String SMValue = eachPerson.getValue();

                if (SMValue.equals("SM")) {
                    System.out.println(eachPerson.getKey() + " is a scrum master");
                }
            }
        }
